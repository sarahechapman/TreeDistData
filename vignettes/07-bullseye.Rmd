...
```{r bullseye-results, echo=FALSE, fig.width=7.2, fig.asp = 4.9/5.1}
PlotFunc <- function() {
...
**Figure 3. Results of 'bullseye' test.** (a–d), success rate for each tree distance metric when comparing pairs of datasets differing in size by 400 base pairs; (e–h), success rates when comparing pairs of datasets differing by 2% in proportion of erroneous tokens; (i–j), accuracy of each method in ranking trees inferred from progressively degraded datasets according to the degree of dataset degradation.
...
dimnames(ret) <- list(tdMdAbbrevs[rownames(ret)], 
                      c("Subsampling: Success rate (50&nbsp;leaves)",
                        "Miscoding: Success rate (50&nbsp;leaves)",
                        "Subsampling: Accuracy (ρ)",
                        "Miscoding: Accuracy (ρ)"))
knitr::kable(ret)
