...
PlotRanks <- function (scoreList, yLab = "Spearman's rho", plotTitle = '', 
  ...
  toPlot <- rev(colnames(rhos))
  toPlot <- toPlot[!toPlot %in% omit]
  
  lapply(toPlot, function (method) {
    if (!is.na(TreeDistCol(method))) {
...
PlotFunc <- function() {
  ...
  PlotRanks(bullseyeMorphScores, "Spearman's rho",
            plotTitle="(i) Subsampling experiment", omit = notPlotted)
  PlotRanks(bullMoDiScores, "",
            plotTitle="(j) Miscoding experiment", omit = notPlotted)
}
