...
distanceDistribution25 <- distanceDistribution25[!rownames(distanceDistribution25) %in% 'nni_t', ]
data("distanceDistribution50", package="TreeDistData")
distanceDistribution50 <- distanceDistribution50[!rownames(distanceDistribution50) %in% 'nni_t', ]
...
PlotR2 <- function (distribution, methodA, methodB) {
  ...
  r2 <- summary(reg)$adj.r.squared
  regCol <- colScale[max(ceiling((r2 - 0.5) * 2000), 1L)]
  text(0.5, 0.5, format(round(r2, 3), nsmall = 2), col = regCol, cex=1.7)
}
